...
const filteredContactGroups: Ref<IContactGroup[]> = ref(chatStore.contactGroups)
  ...
  filteredContactGroups.value = chatStore.contactGroups
    ?.map(group => {
      ...
      newGroup.data = newGroup.data.filter(contact => {
        return (
          contact.name.toLowerCase().includes(searchTextVal.toLowerCase()) ||
          contact.user_id === Number(searchTextVal)
        )
      })
